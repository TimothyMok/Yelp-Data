...
# This Section read load both section of csv that we will use for the data analysis, the data are trim down to 5,000 observation in order to increase the performance of the analysis

```{r Extraction of data from CSV files}
    business<-read.csv("yelp_business.csv",stringsAsFactors = FALSE)
    business<-business[c(1:5000),]
    review<-read.csv("yelp_review.csv",stringsAsFactors = FALSE)
    review<-review[c(1:5000),]
```
...
###############################################################################################
#                    4 - Exploratory analysis on text & visualization                         #
###############################################################################################
...
```{r Explore the top words with bar plot}
  barplot(CorpusSorted[1:30],ylim=c(0,3000),col="orange",las=3,xlab="Word",ylab="Frequencies")
```
...
```{r Pyramaid plot for negative and positive comments, common words comparison}
  ...
  # Text process/TDM creation and include +/- coloumns
    ...
    All<-tm_map(corpus_review_all, tolower)
    All<-tm_map(corpus_review_all, removePunctuation)
    All<-tm_map(corpus_review_all, removeWords, stopwords("english"))
    All<-tm_map(corpus_review_all, removeWords,c("also", "get","like", "company", "made", "can", "im", "dress","just","i"))
    All<-tm_map(corpus_review_all, stemDocument)
